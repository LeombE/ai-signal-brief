...
class ArchiveBuilderTests(unittest.TestCase):
    def test_successful_archive_creation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = build_archive(REPORT, RUN, SOURCES, repo_root / "archive", repo_root=repo_root)
    ...
    def test_archive_index_sorts_dates_descending(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            report_two, run_two = _write_report_and_run_pair(repo_root, "2026-06-25")
            result = build_archive(report_two, run_two, SOURCES, archive_root, repo_root=repo_root)
            index = json.loads(result.index_path.read_text(encoding="utf-8"))
    ...
    def test_unsafe_output_path_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo_root = Path(directory)
            outside_path = repo_root.parent / "outside-archive"

            with self.assertRaises(ArchiveError):
                build_archive(REPORT, RUN, SOURCES, outside_path, repo_root=repo_root)
            self.assertFalse(outside_path.exists())

    def test_quality_gate_failure_blocks_archive_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            repo_root = Path(directory)
            output_path = repo_root / "archive"

            with self.assertRaises(ArchiveError):
                build_archive(REPORT, FIXTURES / "run.mismatched-report-id.json", SOURCES, output_path, repo_root=repo_root)
